...
2. 所属模块：运营后台 / 配送管理
3. 建议路由：
   - `/admin/delivery-management/order-monitor`
...
4. 配送团队负责人需在用户餐段截单前，按展区查看各商家的实时订单量与餐品份数。
...
| roles | array | Y | 角色集合 | Enum: `COURIER, SIGNER, SUPERVISOR`；至少保留 1 个角色 |
...
#### D. `order_monitor_snapshot`（订单实时监控快照）

| 字段 | 类型 | 必填 | 说明 | 校验 |
|---|---|---|---|---|
| areaCode | string | Y | 展区编码 | Enum: `A/B/C/D` |
| merchantId | string | Y | 商家 ID | 必须合法 |
| merchantName | string | Y | 商家名称 | 展示字段 |
| cutoffSlot | enum | Y | 截单时段 | Enum: `CUTOFF_0900/CUTOFF_1000/CUTOFF_1030/CUTOFF_1100/INSTANT` |
| orderCount | number | Y | 订单数 | >= 0 |
| portionCount | number | Y | 餐品份数 | >= 0 |
| refreshedAt | datetime | Y | 刷新时间 | 服务端生成 |
...
1. `GET /api/admin/delivery/order-monitor?date=today`

返回示例：

```json
{
  "refreshedAt": "2026-04-09 10:31:07",
  "activeSlot": "CUTOFF_1000",
  "items": [
    {
      "areaCode": "A",
      "merchantId": "m_101",
      "merchantName": "丽华快餐-A区",
      "cutoffSlot": "CUTOFF_1000",
      "orderCount": 18,
      "portionCount": 52
    }
  ]
...
4. 已取消订单不计入。
5. 用户端同一时刻仅开放一个可下单餐段，因此未开放餐段默认返回 0。
6. 原型中与上次刷新相比发生变化的数字需前端标红。
...
2. `点位商家配置`
3. `配送员管理/名册`
4. `配送员主责配置` (聚合列表查询、快速过滤、批量配置弹窗等功能)
...
3. 角色配置与说明卡：
   ...
   - 【分拣签收员】副标题：“负责在集散地签收餐品并按展厅分拣餐品”；同时此类人员在主责配置页不提供责任配置入口。
   - 【配送主管】副标题：“负责关注配送”；
   - 角色数组 `length >= 1`，不可为空。 
4. 配置提交二次确认：
   - 弹窗展示影响配送员数量；
...
6. `订单实时监控` 交互：
   - 采用极简顶部信息条，仅保留：最新刷新、60秒自动刷新、立即刷新。
   - 主体按展区折叠；展开后为商家横向分时段表格。
   - 列级轻量视觉表达：当前开放餐段轻强调，已切走餐段轻降权，未开放餐段保持常规样式且数值为 0。
   - 原型态允许存在“演示时间”控件，用于切换到 `08:50/09:20/10:05/10:40/11:10` 等典型时间点验证业务逻辑；正式版不作为业务功能要求。

### 6.2 `订单实时监控` 前端规则

1. 当前活跃列由“用户端当前开放餐段”决定，不直接按自然截单时间顺序高亮。
2. 未开放餐段返回值统一展示为 0。
3. 已切走餐段仅保留沉淀量，不继续模拟增长。
4. 红色高亮仅表示相较上次刷新数字发生变化，不表示异常。
...
8. Given 当前演示时间为 `08:50`，When 查看订单实时监控，Then 只有 `10点截单` 列存在主增长订单，后续未开放餐段为 0。
